function getAnimatableGraph(problem, appSettings, graphDescription) {
    ...
    let update = (t, appSettings) => {
        if (appSettings.animating) {
            ...
            // preloading
            let flatteningForceFraction = appSettings.flatteningForceFraction;
            let threeDimForceFraction = appSettings.threeDimForceFraction;
            let previousVertexFocus = appSettings.previousVertexFocus;
            let currentVertexFocus = appSettings.currentVertexFocus;
            let maximumConsideringDepth = appSettings.maximumConsideringDepth;
            let cameraPosition = appSettings.camera.position;
            let friction = appSettings.friction;
            let eps = appSettings.eps;
            let planarityConstant = appSettings.planarityConstant;
            let targetStringLength = appSettings.targetStringLength;
            let stringConstant = appSettings.stringConstant;
            let uniqueStringConstant = appSettings.uniqueStringConstant;
            let repellingConstant = appSettings.repellingConstant;
            ...
            for (let i = 0; i < power; i++) {
                if (!isDiscovered[i] || vertexDistance[i] > maximumConsideringDepth)
                    continue;
                FxArr[i] = appSettings.accelX * -10 - vx[i] * friction;
                FyArr[i] = appSettings.accelY * -10 - vy[i] * friction;
                FzArr[i] = appSettings.accelZ * -10 - vz[i] * friction;
                FwArr[i] = threeDimForceFraction == 1.0 ? 0.0 : (- vw[i] * friction);
                FuArr[i] = threeDimForceFraction == 1.0 ? 0.0 : (- vu[i] * friction);
            }
            ...
            for (let i = 0; i < power; i++) {
                ...
                if (appSettings.flatteningForceFraction > 0.0 && Math.abs(pyi) > eps) {
                    FyArr[i] -= planarityConstant * pyi * appSettings.flatteningForceFraction;
                }
                if (threeDimForceFraction > 0.0) {
                    FwArr[i] -= planarityConstant * pw[i] * threeDimForceFraction;
                    FuArr[i] -= planarityConstant * pu[i] * threeDimForceFraction;
                }
                for (let j = 0; j < i; j++) {
                    if (!isDiscovered[j] || vertexDistance[j] > maximumConsideringDepth) {
                        continue;
                    }
                    let massJtimesMassI = mass[j] * massI;

                    let rx = px[j] - pxi;
                    let ry = appSettings.flatteningForceFraction == 1.0 ? 0.0 : (py[j] - pyi);
                    let rz = pz[j] - pzi;
                    let rw = threeDimForceFraction == 1.0 ? 0.0 : (pw[j] - pwi);
                    let ru = threeDimForceFraction == 1.0 ? 0.0 : (pu[j] - pui);
                    ...
                    elapsedSeconds = repellingConstant * massJtimesMassI / r3;

                    FxArrI += elapsedSeconds * rx;
                    FyArrI += elapsedSeconds * ry;
                    FzArrI += elapsedSeconds * rz;

                    FxArr[j] -= elapsedSeconds * rx;
                    FyArr[j] -= elapsedSeconds * ry;
                    FzArr[j] -= elapsedSeconds * rz;

                    if (threeDimForceFraction < 1.0) {
                        FwArrI += elapsedSeconds * rw;
                        FuArrI += elapsedSeconds * ru;
                    ...
                    if (connectionA[i] == j
                        ...
                        if (partOfShortestWord[i] && partOfShortestWord[j] && (preceedingVertex[j] == i || preceedingVertex[i] == j)) {
                            appropriateStringConstant = uniqueStringConstant;
                        } else {
                            appropriateStringConstant = stringConstant;
                        }
                    }
                    ...
                    let lengthDifferenceFraction = (r - targetStringLength) / r;

                    FxArrI += appropriateStringConstant * rx * lengthDifferenceFraction;
                    FyArrI += appropriateStringConstant * ry * lengthDifferenceFraction;
                    FzArrI += appropriateStringConstant * rz * lengthDifferenceFraction;
                    ...
                    if (threeDimForceFraction < 1.0) {
                        FwArrI += appropriateStringConstant * rw * lengthDifferenceFraction;
                        FuArrI += appropriateStringConstant * ru * lengthDifferenceFraction;
                    ...
                    for (let indexI = 0; indexI < 2; indexI++) {
                        ...
                        for (let j = 0; j < power; j++) {
                            ...
                            for (let indexJ = 0; indexJ < 2; indexJ++) {
                                ...
                                let r = Math.sqrt(rx ** 2 + ry ** 2 + rz ** 2 + rw ** 2 + ru ** 2);
                                if (r <= eps)
                                    continue;
            ...
            let maxOfDepth = Math.max(maximumConsideringDepth, appSettings.previousMaximumConsideringDepth);
            let minOfDepth = Math.min(maximumConsideringDepth, appSettings.previousMaximumConsideringDepth);
            for (let i = 0; i < power; i++) {
                ...
                if (vertexDistLocal > minOfDepth && vertexDistLocal <= maxOfDepth) {
                    if (maximumConsideringDepth > appSettings.previousMaximumConsideringDepth) {
            ...
            if (previousVertexFocus >= 0 && vertexDistance[previousVertexFocus] <= maximumConsideringDepth) {
                sourceCenteringX = px[previousVertexFocus];
                sourceCenteringY = py[previousVertexFocus];
                sourceCenteringZ = pz[previousVertexFocus];
                sourceCenteringW = pw[previousVertexFocus];
                sourceCenteringU = pu[previousVertexFocus];
            }

            if (vertexDistance[currentVertexFocus] > maximumConsideringDepth && vertexDistance[previousVertexFocus] <= maximumConsideringDepth) {
                currentVertexFocus = appSettings.currentVertexFocus = previousVertexFocus;
            }

            if (currentVertexFocus >= 0 && vertexDistance[currentVertexFocus] <= maximumConsideringDepth) {
                targetCenteringX = px[currentVertexFocus];
                targetCenteringY = py[currentVertexFocus];
                targetCenteringZ = pz[currentVertexFocus];
                targetCenteringW = pw[currentVertexFocus];
                targetCenteringU = pu[currentVertexFocus];
            }
            ...
            if (targetFraction == 1.0) {
                appSettings.previousVertexFocus = currentVertexFocus;
            } else {
            ...
            for (let i = 0; i < power; i++) {
                ...
                else if (vertexDistance[i] > maximumConsideringDepth) {
                ...
                sphereGroup[i].lookAt(cameraPosition);
            }
            for (let i = 0; i < power; i++) {
                if (!isDiscovered[i])
                    continue;
                ...
                for (let index = 0; index < 2; index++) {
                    ...
                    if (vertexDistance[i] > maximumConsideringDepth
                        || vertexDistance[connectionArrayIndex] > maximumConsideringDepth) {
                    ...
                    if (connectionArrayIndex != i) {
                        ...
                        if (arrowTransitions[2 * i + index] != undefined) {
                            ...
                            arrowTransitions[2 * i + index].lookAt(cameraPosition);
                        }
                    }
                }
            }
